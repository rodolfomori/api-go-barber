...
class AppointmentController {
  // index é o metodo de listagem padrao
  async index(req, res) {
  ...
  async store(req, res) {
    ...
    // check provider is the same id the client
    ...
    await Notification.create({
      content: `Novo agendamento do ${user.name} para dia ${formattedDate}`,
      user: provider_id,
    });
  ...
  async delete(req, res) {
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res.status(401).json({
        error: 'You can only cancel appointments2 hours advancee',
      });
    }
